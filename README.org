...
If user option =org-marginalia-use-org-id= is non-nil (default), Org-marginalia will create a link back to the source note with using an Org-ID link instead of a
normal file link.
