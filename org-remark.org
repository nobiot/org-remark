...
~org-remark-notes-file-name~ also takes a function as a value, so you have a wide range of customizability. In ~customize~, the default function to be used is ~org-remark-notes-file-name-function~, which adds =-notes.org= as a suffix to the source file's name without the extension. For example, for =my-main-file.txt= Org-remark will create and store highlights in  =my-main-file-notes.org=.
